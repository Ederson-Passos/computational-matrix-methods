...
\end{equation}
To verify the factorization, the reconstruction residual was calculated as $\|A - G G^T\|_2 \approx 1.096 \times 10^{-14}$, confirming reconstruction to near machine precision.

Since $A$ is SPD, the $2$-norm condition number can be computed as $\kappa_2(A) = \lambda_{\max}(A) / \lambda_{\min}(A)$. Using the actual computed matrix, the extreme eigenvalues are approximately $\lambda_{\min} = 10.008436$ and $\lambda_{\max} = 292.550973$, and therefore $\kappa_2(A) = 29.230438$ \cite{golub2013matrix}.
...
The resulting linear system $M_{orig} T_{orig} = s_{orig}$ was solved computationally. The numerical residual is $\|M_{orig} T_{orig} - s_{orig}\|_2 \approx 1.8253 \times 10^{-15}$. This negligible residual verifies that the generated discrete linear system is algebraically solvable and was solved accurately from a strict algebraic perspective. However, this algebraic residual is fundamentally distinct from and does not imply consistency with the stated continuous PDE.
...
\begin{figure}[ht]
    ...
    \caption{One-dimensional steady-state heat-conduction configuration for the generated Neumann--Dirichlet boundary conditions.}
...
\end{equation}
This first-order approximation at the boundary limits the global convergence of the corrected scheme to first order, which is sufficient to establish a mathematically consistent discrete representation.
...
\autoref{fig:temperature-comparison} visually emphasizes the qualitative distinction between the formulations. The corrected formulation reproduces the qualitative features of the analytical solution and approaches the analytical profile as the grid is refined. The interior stencil of the original formulation approximates $T''$ with the opposite sign and the source term is improperly scaled by $h^2$.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/problem2_convergence.pdf}
    \caption{Observed convergence of the corrected formulation under grid refinement; the dashed line denotes a first-order reference slope.}
    \label{fig:problem2-convergence}
\end{figure}

The computed convergence rate in the $L_\infty$-norm is approximately $1.00$, and in the $L_2$-norm is approximately $1.03$. The observed first-order convergence is consistent with the first-order forward-difference approximation used for the Neumann boundary condition \cite{leveque2007finite}.
